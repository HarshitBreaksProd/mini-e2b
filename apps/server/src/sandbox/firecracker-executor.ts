...
const waitForVMReady = async (vmName: string, maxAttempts = 100, delayMs = 500) => {
  for (let i = 0; i < maxAttempts; i++) {
    try {
      const { stdout } = await execAsync(`ignite ps --filter "{{.Name}}==${vmName}" --template "{{.Status}}"`);
  ...
  throw new Error(`VM ${vmName} did not become ready within ${maxAttempts} attempts`);
}
